...
STRATEGIST_TOOL_SCHEMA = {
    ...
    "input_schema": {
        ...
        "required": ["searchQuery", "currentJobTitles", "locations", "reasoning"],
    },
}
...
PM_TOOL_SCHEMA = {
    ...
    "input_schema": {
        ...
        "required": ["diagnosis", "searchQuery", "currentJobTitles", "locations", "new_lesson"],
    },
}
...
def load_recruiter_feedback(apify_token: str) -> tuple[list, str | None]:
    ...
    try:
        client = ApifyClient(apify_token)
        store = client.key_value_stores().get_or_create(name=KNOWN_ISSUES_STORE_NAME)
        record = client.key_value_store(store["id"]).get_record(RECRUITER_FEEDBACK_RECORD_KEY)
        value = record.get("value") if record else None
        return (value if isinstance(value, list) else []), None
    except Exception as e:
        return [], f"{type(e).__name__}: {e}"
...
def save_recruiter_feedback(apify_token: str, feedback: list) -> str | None:
    ...
    try:
        client = ApifyClient(apify_token)
        store = client.key_value_stores().get_or_create(name=KNOWN_ISSUES_STORE_NAME)
        client.key_value_store(store["id"]).set_record(RECRUITER_FEEDBACK_RECORD_KEY, feedback, content_type="application/json")
        return None
    except Exception as e:
        return f"{type(e).__name__}: {e}"
...
def vacancy_key(job_description: str) -> str:
    ...
    а хеш детерміновано прив'язаний до точного змісту опису."""
    digest = hashlib.sha256(job_description.strip().encode("utf-8")).hexdigest()[:16]
...
def load_seen_urls(apify_token: str, job_description: str) -> tuple[set, str | None]:
    ...
    try:
        ...
        record = client.key_value_store(store["id"]).get_record(vacancy_key(job_description))
        value = record.get("value") if record else None
...
def filter_relevant_xray_results(xray_results: list, search_params: dict) -> list:
    ...
    if not titles and search_params.get("searchQuery"):
        # Витягуємо "голі" слова з boolean-рядка (прибираємо OR/лапки), щоб
        # було з чим порівнювати текст сніпета
        titles = [t.strip(' "') for t in re.split(r'\bOR\b', search_params["searchQuery"]) if t.strip(' "')]
    keywords = [t.lower() for t in titles if t]
    if not keywords:
...
def agent_evaluator(client, job_description: str, profile_text: str, stop_list: list, allow_belarusians: bool, recruiter_feedback: list = None) -> dict:
...
def process_single_candidate(claude_client, profile: dict, source: str, ashby_api_key: str,
    ...
    eval_result = agent_evaluator(claude_client, job_description, profile_text, stop_list, allow_belarusians, recruiter_feedback=recruiter_feedback)
...
def run_one_iteration(claude_client, apify_token: str, recruiter_note: str):
    ...
    while len(st.session_state.approved_candidates) < st.session_state.target_count:
            ...
            if st.button("💾 Зберегти фідбек для навчання AI"):
                if not apify_token:
                    ...
                    if not new_entries:
                        ...
                        combined = st.session_state.recruiter_feedback + new_entries
                        save_error = save_recruiter_feedback(apify_token, combined)
                        if save_error:
