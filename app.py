...
def run_one_iteration(claude_client, apify_token: str, recruiter_note: str):
    ...
    profiles = run_apify_search(apify_token, search_params, st.session_state.max_profiles, start_page=start_page)
    # Наступного разу для цього ж самого запиту (searchQuery+locations) тягнемо
    # НАСТУПНУ сторінку видачі, а не ту саму - інакше ітерації просто повторно
    # повертають той самий верх видачі, і "нових" кандидатів майже нема.
    ...
    log.append(f"📥 LinkedIn-пошук (Apify, сторінка {start_page}): знайдено {len(profiles)} профілів, з них {len(new_profiles)} нових")
