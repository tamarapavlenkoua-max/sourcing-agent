...
# БАГ, ЯКИЙ ЦЕ ВИПРАВЛЯЄ: known_issues раніше передавався в промпт
# Strategist і PM-агента ПОВНІСТЮ, без жодного ліміту (на відміну від
# recruiter_feedback вище, де ліміт для промпту вже був). Зі зростанням
# спільної пам'яті команди (до MAX_KNOWN_ISSUES_STORED=200 у сховищі)
# КОЖЕН виклик Strategist і PM-агента поступово дорожчав би сам собою,
# незалежно від prompt caching. Найновіші уроки зазвичай найактуальніші.
MAX_KNOWN_ISSUES_IN_PROMPT = 30

# ЕКОНОМІЯ ТОКЕНІВ: усі json.dumps(...) для даних, які йдуть В ПРОМПТ Claude
# (known_issues, recruiter_feedback, search_history, previous_attempts,
# evaluator_result) - тепер компактні (separators=(",", ":")), без
# indent=2. Pretty-print з відступами й переносами рядків зручний для
# людини, але Claude однаково добре читає компактний JSON, а відступи й
# переноси - це чисті зайві токени на КОЖНОМУ виклику. Не стосується
# даних, які показуються рекрутерці в інтерфейсі (там форматування не
# змінювали).
...
# ЕКОНОМІЯ ТОКЕНІВ: поля, які актор harvestapi/linkedin-profile-search
# реально повертає в КОЖНОМУ профілі (перевірено через справжню
# output-схему актора), але на які Evaluator/Critic ніде в своїх
# інструкціях не посилаються - лого компаній/шкіл (об'єкти з кількома
# розмірами картинок), фото профілю, внутрішні ID актора/LinkedIn
# (companyId, objectUrn, schoolId, experienceGroupId тощо), дублікати
# локалі/пагінації, і "currentPositions" (з "s" на кінці) - окреме поле,
# що дублює ту саму інформацію, що вже є в currentPosition + experience,
# просто в іншому форматі. Це чистий шум: не несе жодного сигналу для
# оцінки кандидата, але роздуває розмір КОЖНОГО виклику Evaluator/Critic.
PROFILE_NOISE_KEYS = {
    "companyLogo", "schoolLogo", "profilePicture", "coverPicture", "photo",
    "pictureUrl", "givenByPicture", "image", "_meta", "moreProfiles",
    "profileLocales", "multiLocaleHeadline", "primaryLocale", "companyId",
    "companyUniversalName", "schoolId", "experienceGroupId", "objectUrn",
    "profileIdInSearch", "id", "currentPositions", "emails",
}


def strip_profile_noise(value):
    """Рекурсивно прибирає PROFILE_NOISE_KEYS з будь-якої глибини структури -
    шум ховається і в experience[], і в education[], і в
    receivedRecommendations[] тощо, не тільки на верхньому рівні профілю."""
    if isinstance(value, dict):
        return {k: strip_profile_noise(v) for k, v in value.items() if k not in PROFILE_NOISE_KEYS}
    if isinstance(value, list):
        return [strip_profile_noise(item) for item in value]
    return value
...
def profile_to_compact_text(p: dict, max_chars: int = 6000) -> str:
    ...
    Перед серіалізацією прибираємо PROFILE_NOISE_KEYS (лого, фото, внутрішні
    ID) - економія токенів БЕЗ жодної втрати реального сигналу для оцінки,
    бо Evaluator/Critic на ці поля й так ніколи не посилаються."""
    p = strip_profile_noise(p)
    try:
    ...
    for list_field in ("experience", "education"):
        ...
        if not isinstance(items, list) or not items:
            continue
        while items and len(json.dumps(trimmed, ensure_ascii=False, separators=(",", ":"))) > max_chars:
...
def ask_claude_tool(client: anthropic.Anthropic, system_prompt: str, user_prompt: str, tool_schema: dict, retries: int = 2) -> dict:
    ...
    ЕКОНОМІЯ ВАРТОСТІ - prompt caching: system_prompt і tool_schema для
    кожного агента ІДЕНТИЧНІ на кожному виклику в межах сесії (змінюється
    тільки user_prompt) - а Evaluator/Critic викликаються по 20-45+ разів за
    одну ітерацію. Раніше ми платили повну ціну за системний промпт КОЖНОГО
    разу. Тепер system і tool_schema позначені `cache_control: ephemeral` -
    Anthropic кешує їх на 5 хвилин (оновлюється при кожному звертанні), і
    кожен наступний виклик з тим самим текстом коштує ~10% від звичайної
    ціни за ці токени замість 100% (перевірено з поточною документацією
    Anthropic: GA-фіча, без спеціальних beta-заголовків, поріг кешування -
    від ~1024 токенів на блок для Sonnet-класу моделей; наші системні
    промпти агентів довші за цей поріг)."""
    last_error = None
    tool_name = tool_schema["name"]
    cached_tool_schema = {**tool_schema, "cache_control": {"type": "ephemeral"}}
...
бути порожньо):
{json.dumps((known_issues or [])[-MAX_KNOWN_ISSUES_IN_PROMPT:], ensure_ascii=False, separators=(",", ":"))}

Фідбек рекрутера - кого і чому відхилили вручну раніше (може бути порожньо):
{json.dumps((recruiter_feedback or [])[-MAX_FEEDBACK_ENTRIES_IN_PROMPT:], ensure_ascii=False, separators=(",", ":"))}

Історія попередніх ітерацій цього пошуку (може бути порожньою):
{json.dumps(search_history, ensure_ascii=False, separators=(",", ":"))}
"""
...
{json.dumps(previous_attempts, ensure_ascii=False, separators=(",", ":"))}
...
def run_apify_search(apify_token: str, search_params: dict, max_items: int, start_page: int = 1, profile_scraper_mode: str = "Full") -> list:
    """ДВОЕТАПНИЙ СКРАПІНГ (economy) - перевірено через реальну input/pricing-схему
    актора harvestapi/linkedin-profile-search (Apify API, не здогадка): 'Search page
    results' (короткі картки, Short-режим) і 'Full profile' (повні дані, Full-режим) -
    це ДВА окремі платні events того самого актора, ~$0.004/профіль кожен. Раніше ми
    завжди платили ОБИДВА events за КОЖЕН профіль (~$0.008), включно з тими, кого
    filter_relevant_profiles (нижче в run_one_iteration) все одно відсіє за заголовком.
    Тепер run_one_iteration викликає цю функцію з profile_scraper_mode='Short', проганяє
    результат через filter_relevant_profiles, і ЛИШЕ для тих, хто пройшов, окремо
    викликає fetch_linkedin_profiles_by_url (та сама функція, що вже й так
    використовується для Google X-ray) - другий, дорожчий event, але тепер тільки для
    вужчого списку, а не для всіх. Параметр НЕ хардкодиться, щоб можна було вимкнути
    двоетапний режим чекбоксом у сайдбарі і повернутись до старої поведінки (Full
    одразу), якщо колись виявиться, що Short-картки недостатньо заповнені для
    keyword-фільтру (це поки не перевірено на реальному пошуку - тільки по документації
    актора)."""
    client = ApifyClient(apify_token)
    run_input = {
...
EVALUATOR_TOOL_SCHEMA = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "reason_tag": {
                "type": "string",
                "enum": ["Direct Match", "Missing Stack Info", "Domain Mismatch", "Overqualified", "Hard Blocker Hit", "Clear Mismatch"],
            },
            "confidence_score": {"type": "number"},
            "pros": {"type": "array", "items": {"type": "string"}},
            "gaps": {"type": "array", "items": {"type": "string"}},
            "recruiter_action_item": {"type": "string"},
            "summary": {"type": "string"},
        },
        "required": ["hard_stop", "hard_stop_reason", "stop_list_hit", "ex_stop_list_company",
                     "location_note", "total_yoe", "relevant_yoe", "grade", "tier",
                     "reason_tag", "confidence_score", "pros", "gaps", "recruiter_action_item", "summary"],
    },
}
...
def agent_evaluator(client, job_description: str, profile_text: str, stop_list: list, allow_belarusians: bool, recruiter_feedback: list = None, donor_list: list = None) -> dict:
    feedback_block = json.dumps((recruiter_feedback or [])[-MAX_FEEDBACK_ENTRIES_IN_PROMPT:], ensure_ascii=False, separators=(",", ":"))
    system_prompt = f"""Ти - Senior Sourcing Specialist & Talent Evaluator у SKELAR: суворий там, де
є чіткі hard-правила, і справедливий там, де йдеться про реальну неповноту
LinkedIn-даних, а не про фактичну невідповідність кандидата.

ФІЛОСОФІЯ ОЦІНКИ:
1. Контекст важливіший за ключові слова. LinkedIn-профіль - це короткий
   витяг, а не детальне CV.
2. Відсутність згадки конкретного інструменту чи фреймворку в тексті НЕ
   означає, що кандидат ним не володіє, якщо рівень компаній, тайтл і
   грейд відповідають вимогам.
3. "Benefit of the doubt" (сумнів на користь кандидата) застосовується
   ТІЛЬКИ до невизначеності в навичках/досвіді - якщо вагаєшся між tier
   "B" і "C" через таку невизначеність, обирай "B". Це правило НІКОЛИ не
   поширюється на HARD-STOP критерії нижче (Росія, поточна стоп-компанія):
   там сумнівів на користь кандидата не буває - є чіткий доказ у даних
   профілю - hard_stop = true, немає доказу - hard_stop = false, і зважати
   на "можливо" не потрібно.
...
- tier:
  - "A" (Direct Match) - кандидат повністю відповідає hard-вимогам
    вакансії, сильний релевантний досвід у відповідних компаніях/продуктах.
  - "B" (High Potential / Quick Check) - потрібний грейд, тайтл та
    індустріальний бекграунд є, але лишається 1-2 конкретні прогалини
    (напр. не вказаний конкретний фреймворк, нечітка деталь локації, чи
    суміжний, а не точно той самий стек). Це РЕАЛЬНИЙ кандидат, якого
    рекрутерка має побачити і перевірити власними очима, а не автоматично
    відкинутий - він так само йде у фінальний список, просто з позначкою
    "B" і конкретним питанням для швидкої перевірки (recruiter_action_item).
  - "C" (Clear Mismatch) - критична невідповідність: брак років досвіду на
    порядок, зовсім інша спеціалізація, або невідповідна локація/мова, якщо
    вона прямо вказана як стопер у вакансії.

КРИТИЧНО ВАЖЛИВО - неповнота даних LinkedIn ≠ невідповідність ролі
(перевірено на реальних кандидатах: рекрутерка вручну перевірила кількох
"B"-кандидатів і підтвердила, що вони насправді ідеальний збіг - Evaluator
занижував tier не через реальну невідповідність ролі, а через те, що
профіль неідеально заповнений, а це нормальна властивість LinkedIn, не
сигнал про слабкість кандидата):
- LinkedIn-профілі РІДКО заповнені ідеально. Старі позиції часто без
  точних дат (`startDate`/`endDate`), давні ролі згадані побіжно в
  описі чи skills, а не окремими записами `experience`. Це типова
  реальність даних, а не привід автоматично занижувати tier чи grade.
- Якщо ПОТОЧНА позиція (`currentPosition`) явно відповідає вакансії за
  тайтлом і рівнем, а решта досвіду виглядає правдоподібно (навіть без
  повних дат) - це нормальні підстави для tier "A", а не привід
  знижувати до "B" через "не можу перевірити кожен рік".
- Коли треба оцінити total_yoe/relevant_yoe, а частина записів без дат -
  дай розумну, обґрунтовану оцінку на основі доступних сигналів
  (headline, поточна посада, кількість згаданих попередніх ролей,
  `registeredAt` як орієнтовна нижня межа кар'єри) і ЯВНО напиши в
  summary, що це оцінка через неповні дані, а не точний підрахунок.
  Чесна, позначена оцінка - це нормально; занижувати tier "про всяк
  випадок" через відсутність ідеальних даних - ні.
- "B" тримай для випадків, коли РОЛЬ чи РІВЕНЬ справді не повністю
  збігаються (напр. досвід в іншому домені, явно нижчий грейд, немає
  ключової навички з вакансії) - а не для випадків, коли роль і рівень
  збігаються, але дані просто неідеально структуровані.

ДОДАТКОВІ ЕВРИСТИКИ (перевірені на практиці в іншому нашому сорсинговому
...
ДОДАТКОВІ ПОЛЯ (потрібні для КОЖНОГО кандидата, включно з tier "C" -
не пропускай їх тільки тому, що кандидат явно слабкий):
- reason_tag: одне з "Direct Match" (A), "Missing Stack Info" або
  "Domain Mismatch" або "Overqualified" (типові причини для B),
  "Hard Blocker Hit" (якщо hard_stop=true) або "Clear Mismatch" (C без
  hard-stop). Обирай той, що найточніше описує ГОЛОВНУ причину рішення.
- confidence_score: число 0-100 - наскільки ти впевнений у своєму
  рішенні щодо tier, з огляду на повноту даних профілю. Низька впевненість
  (напр. 50-60) при нормальному tier - це чесний сигнал рекрутерці, а не
  привід самому знижувати tier "про всяк випадок".
- pros: масив з 1-2 найсильніших сторін кандидата САМЕ під цю вакансію
  (коротко, конкретно, без води).
- gaps: масив з 1-2 білих плям чи сумнівів (для tier "A" може бути
  порожнім масивом [], якщо реальних сумнівів немає).
- recruiter_action_item: ТІЛЬКИ для tier "B" - ОДНЕ чітке, конкретне
  питання чи дія, яку рекрутерка може перевірити в LinkedIn-профілі за
  5 секунд, щоб самій вирішити, чи піднімати кандидата до "A" (напр.
  "Перевір About - чи згадує Looker/Tableau" або "Уточни, чи локація
  Warsaw - це релокація чи вже постійне місце"). Для tier "A" і "C" -
  залиш це поле порожнім рядком "".
...
залишай як "" якщо неактуальні, порожні масиви як [] - а не пропускай
поле взагалі)."""
    user_prompt = f"""Опис вакансії:
{job_description}
...
5. Якщо tier = "B" через "benefit of the doubt" (сумнів на користь
   кандидата) - чи це справді невизначеність у навичках/досвіді, а НЕ
   замаскований hard-stop? Якщо в даних профілю є чіткий доказ hard-stop
   критерію (Росія за location, або стоп-компанія САМЕ в currentPosition),
   а Evaluator все одно поставив tier "B" замість hard_stop=true - це
   критична помилка, познач її як issue.

Нові поля Evaluator-а (reason_tag, confidence_score, pros, gaps,
recruiter_action_item) НЕ є предметом твоєї перевірки самі по собі - не
став issue тільки за стиль чи формулювання pros/gaps. Дивись на них лише
якщо вони прямо СУПЕРЕЧАТЬ hard_stop/tier рішенню (напр. reason_tag
"Direct Match" при tier "C", або recruiter_action_item непорожній при
tier "A"/"C") - тоді це ознака внутрішньої неузгодженості, і це issue.

КРИТИЧНО ВАЖЛИВО - неповні дані LinkedIn ≠ помилка Evaluator-а (перевірено
на практиці: раніше Скептик відхиляв кандидатів ТІЛЬКИ за те, що старі
позиції в experience без точних дат чи описані побіжно - а не тому, що
Evaluator реально щось наплутав чи вигадав). LinkedIn-профілі рідко
заповнені ідеально - відсутність `startDate`/`endDate` на давніх позиціях
чи стислий опис старого досвіду - це нормальна властивість даних, а не
галюцинація. Став issue ТІЛЬКИ якщо:
- Evaluator стверджує щось, що прямо СУПЕРЕЧИТЬ видимим даним (напр. каже
  "немає збігу зі стоп-листом", хоча стоп-компанія явно є в
  currentPosition), або
- цифра (total_yoe/relevant_yoe) виглядає ЯВНО неправдоподібною чи
  видуманою (наприклад, набагато більшою, ніж будь-що видиме в профілі, і
  без жодного пояснення, звідки вона), або
- Evaluator НЕ позначив явно, що це оцінка через неповні дані, хоча дані
  справді неповні (тобто подав приблизну цифру як точний факт).
Якщо Evaluator дав розумну, чесно позначену як приблизну оцінку на основі
наявних сигналів (headline, поточна позиція, кількість згаданих ролей) -
це НЕ привід для issue, навіть якщо ти сам не можеш перевірити її на
100%. Твоя робота - ловити реальні помилки й вигадки, а не карати
Evaluator-а за те, що дані від LinkedIn неповні.

pass = true ставиш, якщо реальних проблем (за критеріями вище) не
знайдено - НЕ вимагай "перфектного скору" там, де єдина причина сумніву
це неповнота вихідних даних, а не логіка Evaluator-а. Виклич інструмент
submit_critique з результатом."""
    user_prompt = f"""Опис вакансії:
{job_description}
...
{json.dumps(evaluator_result, ensure_ascii=False, separators=(",", ":"))}
"""
...
with st.sidebar:
    ...
    enable_two_stage_scraping = st.checkbox(
        "🔀 Двоетапний скрапінг основного каналу (economy)",
        value=True,
        help="Спочатку тягне ЛЕГКІ картки пошуку (Short-режим актора harvestapi/"
        "linkedin-profile-search - ім'я, заголовок, поточна компанія, локація, URL), "
        "проганяє їх через безкоштовний keyword-фільтр (той самий, що вже працює нижче), "
        "і ЛИШЕ для тих, хто пройшов, окремо дотягує повний профіль (той самий актор, "
        "що вже використовується для Google X-ray). Перевірено по реальному прайсингу "
        "актора: 'Search page results' і 'Full profile' - два окремі платні events, "
        "~$0.004/профіль кожен - тобто раніше платили ОБИДВА за кожного, тепер другий - "
        "тільки за тих, хто пройшов фільтр. 🧪 Ще НЕ перевірено на реальному пошуку, чи "
        "Short-картки мають достатньо заповнені headline/currentPosition для фільтру - "
        "перший запуск варто уважно порівняти зі старою поведінкою (вимкни чекбокс, щоб "
        "повернутись до Full одразу, якщо результат виглядає гірше).",
    )
...
def lock_in_and_start(job_description: str, target_count: int):
    ...
    st.session_state.enable_two_stage_scraping = enable_two_stage_scraping
...
def count_a_tier(rows: list) -> int:
    """Скільки серед уже зібраних кандидатів - саме tier "A" (ідеальний
    збіг). Ціль пошуку (target_count, "Скільки ідеальних (A-tier)
    кандидатів потрібно") рахується і надалі ЛИШЕ по A-tier, навіть після
    того як B-tier кандидати теж почали потрапляти у approved_candidates
    для перегляду рекрутеркою - інакше "10 ідеальних" непомітно
    перетворилось би на "10 будь-яких показаних", і пошук зупинявся б
    зарано, не досягнувши реальної цілі."""
    return sum(1 for row in rows if row.get("Tier") == "A")
...
def process_single_candidate(claude_client, profile: dict, source: str, ashby_api_key: str,
    ...
    # Раніше тут пропускали все, крім tier "A" - у результаті нормальні
    # B-tier кандидати ("непоганий, але не ідеальний") губились повністю,
    # хоча Evaluator їх явно НЕ бракував. Тепер B так само йде у фінальний
    # список (з позначкою tier і конкретним питанням для рекрутерки), і
    # тільки C (слабкий збіг) відсіюється тут без виклику Critic-а.
    if tier not in ("A", "B"):
        log_lines.append(f"🟡 {TEAM['evaluator']}: {name} - {tier or '?'}-tier, слабкий збіг, пропускаю")
        return None, log_lines
    ...
    pros = eval_result.get("pros") or []
    gaps = eval_result.get("gaps") or []
    summary_parts = [p for p in [eval_result.get("summary", "")] if p]
    if pros:
        summary_parts.append("Плюси: " + "; ".join(pros))
    if gaps:
        summary_parts.append("Прогалини: " + "; ".join(gaps))
    ...
    candidate_row = {
        ...
        "Питання рекрутеру": eval_result.get("recruiter_action_item", "") if tier == "B" else "",
        "Причина відхилення": "",
        "Своя причина": "",
    }
    log_lines.append(f"✅ {TEAM['critic']} підтвердив {name} ({source}) - кандидат {tier}-tier")
...
def run_one_iteration(claude_client, apify_token: str, recruiter_note: str):
    ...
    still_needed = st.session_state.target_count - count_a_tier(st.session_state.approved_candidates)
    ...
    for attempt in range(1, MAX_APIFY_ATTEMPTS_PER_ITERATION + 1):
        ...
        main_channel_mode = "Short" if st.session_state.get("enable_two_stage_scraping", True) else "Full"
        profiles, apify_error = run_apify_search(
            apify_token, search_params, st.session_state.max_profiles,
            start_page=start_page, profile_scraper_mode=main_channel_mode,
        )
    ...
    # ДВОЕТАПНИЙ СКРАПІНГ (economy): якщо основний канал тягнув тільки Short-картки
    # (див. run_apify_search вище), relevant_profiles зараз - це легкі картки БЕЗ
    # experience/education/skills, яких потребує Evaluator. Дотягуємо повний профіль
    # ОКРЕМИМ платним викликом (той самий актор і та сама функція, що вже й так
    # використовується для Google X-ray нижче) - але тепер тільки для тих, хто вже
    # пройшов безкоштовний keyword-фільтр вище, а не для всіх 20-25 знайдених.
    if st.session_state.get("enable_two_stage_scraping", True):
        survivor_urls = [url for url in (get_profile_url(p) for p in relevant_profiles) if url]
        if survivor_urls:
            full_profiles, full_fetch_error = fetch_linkedin_profiles_by_url(apify_token, survivor_urls)
            if full_fetch_error:
                log.append(f"❌ ПОМИЛКА дотягування повних профілів (етап 2): {full_fetch_error}")
                main_channel_profiles = []
            else:
                skipped_before_full_fetch = len(new_profiles) - len(survivor_urls)
                log.append(
                    f"💰 Двоетапний скрапінг: повний профіль дотягнуто лише для {len(survivor_urls)} "
                    f"з {len(new_profiles)} знайдених - економія ~{skipped_before_full_fetch} платних "
                    "'Full profile' викликів цієї ітерації"
                )
                main_channel_profiles = full_profiles
        else:
            main_channel_profiles = []
    else:
        # Двоетапний режим вимкнено в сайдбарі - relevant_profiles уже повні дані
        # (main-канал тягнув Full одразу, як і раніше).
        main_channel_profiles = relevant_profiles

    candidate_pool = [(p, "LinkedIn пошук") for p in main_channel_profiles]
    ...
    while count_a_tier(st.session_state.approved_candidates) < target_for_this_pass:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CANDIDATE_WORKERS) as executor:
            ...
            for future in futures:
                ...
                if candidate_row:
                    ...
                    log.append(
                        f"   → {count_a_tier(st.session_state.approved_candidates)}/{st.session_state.target_count} A-tier "
                        f"(+{len(st.session_state.approved_candidates) - count_a_tier(st.session_state.approved_candidates)} B-tier)"
                    )
    ...
    a_tier_so_far = count_a_tier(st.session_state.approved_candidates)
    if a_tier_so_far >= st.session_state.target_count:
        st.session_state.stage = "done"
        log.append(f"✅ Ціль досягнута: {a_tier_so_far}/{st.session_state.target_count} A-tier")
    elif iteration >= st.session_state.max_iterations:
        st.session_state.stage = "done"
        log.append(
            f"⏹️ Ліміт ітерацій вичерпано ({st.session_state.max_iterations}), "
            f"знайдено {a_tier_so_far}/{st.session_state.target_count} A-tier "
            f"(+{len(st.session_state.approved_candidates) - a_tier_so_far} B-tier показано на додачу)"
        )
...
if st.session_state.stage == "active":
    ...
    if st.session_state.trigger_run:
        ...
        preview_columns = ["Tier", "Ім'я", "LinkedIn", "Поточна роль", "Грейд", "Джерело", "AI summary"]
        preview_rows = [
        ...
        _a_tier_now = count_a_tier(st.session_state.approved_candidates)
        st.info(
            f"Ітерація {st.session_state.iteration}/{st.session_state.max_iterations} завершена. "
            f"Знайдено {_a_tier_now}/{st.session_state.target_count} A-tier кандидатів "
            f"(+ {len(st.session_state.approved_candidates) - _a_tier_now} B-tier на перегляд)."
        )
...
if st.session_state.stage == "done":
    _a_tier_final = count_a_tier(st.session_state.approved_candidates)
    if 0 < _a_tier_final < st.session_state.get("target_count", 0):
        st.warning(
            f"Зупинено на {_a_tier_final}/{st.session_state.target_count} A-tier кандидатів "
            f"(+ {len(st.session_state.approved_candidates) - _a_tier_final} B-tier на додачу). "
    ...
    if st.session_state.approved_candidates:
        st.divider()
        st.subheader(
            f"Результати: {len(st.session_state.approved_candidates)} кандидатів "
            f"({_a_tier_final} A-tier + {len(st.session_state.approved_candidates) - _a_tier_final} B-tier)"
        )
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "Tier": st.column_config.TextColumn("Tier", help="A = ідеальний збіг, B = непоганий, перевір 'Питання рекрутеру'."),
                "Питання рекрутеру": st.column_config.TextColumn(
                    "Питання рекрутеру",
                    help="Заповнено тільки для B-tier - конкретне питання, яке можна перевірити в LinkedIn за 5 секунд.",
                ),
            ...
            disabled=["Tier", "Ім'я", "LinkedIn", "Поточна роль", "Грейд", "Джерело", "Ashby статус", "AI summary", "Питання рекрутеру"],
            use_container_width=True,
            hide_index=True,
        )
        ...
        with col_export:
            if st.button(f"📤 Експортувати обраних ({len(selected)}) у CSV для Ashby"):
                if not selected:
                    ...
                    writer.writerow(["Name", "LinkedIn URL", "Current Role", "Grade", "Tier", "Source", "Notes", "Recruiter Check"])
                    for row in selected:
                        writer.writerow([
                            row["Ім'я"], row["LinkedIn"], row["Поточна роль"], row["Грейд"],
                            row.get("Tier", ""), row["Джерело"], row["AI summary"], row.get("Питання рекрутеру", ""),
                        ])
                    st.download_button(
                        "⬇️ Завантажити CSV",
                        data=buf.getvalue(),
