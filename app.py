...
цей тайтл з currentJobTitles і не повертайся до нього.

Відповідай ТІЛЬКИ у форматі JSON, без пояснень навколо:
{
  "searchQuery": "короткий пошуковий запит (fuzzy search), напр. 'Growth Marketing Manager'",
  "currentJobTitles": ["список актуальних тайтлів, максимум 5"],
...
def run_apify_search(apify_token: str, search_params: dict, max_items: int) -> list:
    ...
    if search_params.get("locations"):
        run_input["locations"] = search_params["locations"]
    ...
    try:
        ...
        st.error(f"Помилка виклику Apify: {e}")
        return []
...
def build_xray_query(search_params: dict) -> str:
    ...
    locations = (search_params.get("locations") or ["Ukraine"])[:3]
...
def run_google_xray_search(apify_token: str, query: str) -> list:
    ...
    try:
        ...
        st.warning(f"Google X-ray пошук не спрацював цього разу: {e}")
        return []
...
def fetch_linkedin_profiles_by_url(apify_token: str, urls: list) -> list:
    ...
    try:
        ...
        st.warning(f"Не вдалось дотягнути профілі, знайдені через Google X-ray: {e}")
        return []
...
1. Кандидат з Росії, живе в Росії, або явно вказує російське громадянство
   без ознак релокації/протидії режиму - HARD STOP.
   Білоруси дозволені ТІЛЬКИ якщо allow_belarusians = {allow_belarusians} і
   вони явно перебувають ЗА межами Білорусі.
2. Поточна або минула компанія кандидата збігається (навіть частково,
   без урахування регістру) з будь-якою компанією зі стоп-листа: {stop_list}
   -> HARD STOP.

Якщо hard_stop не спрацював - оціни:
- total_yoe: загальний досвід роботи в роках (число)
- relevant_yoe: досвід САМЕ в релевантній для вакансії сфері/ролі (число).
  Якщо загальний досвід 5 років, але релевантний тільки 1 рік - грейд
  визначається по relevant_yoe (тобто це Junior, а не Middle/Senior).
...
{{
  ...
  "stop_list_hit": "назва компанії зі стоп-листа якщо збіг, інакше пусто",
  "location_note": "що саме в профілі вказує на локацію/громадянство",
  "total_yoe": число,
  "relevant_yoe": число,
...
1. Чи Evaluator дійсно звірив компанії кандидата зі стоп-листом {stop_list},
   чи просто написав "false" навмання?
2. Чи справді є докази в профілі, що людина НЕ з Росії (а не просто відсутність
   згадки про Росію)?
...
def run_one_iteration(claude_client, apify_token: str, recruiter_note: str):
    ...
    log.append(
        f"🔎 Запит: `{search_params.get('searchQuery')}` | тайтли: {search_params.get('currentJobTitles')} | "
        f"локації: {search_params.get('locations')}"
    )

    profiles = run_apify_search(apify_token, search_params, st.session_state.max_profiles)
    ...
    log.append(f"📥 LinkedIn-пошук (Apify): знайдено {len(profiles)} профілів, з них {len(new_profiles)} нових")
    ...
    if st.session_state.get("enable_google_xray"):
        ...
        xray_urls = run_google_xray_search(apify_token, xray_query)
        ...
        xray_profiles = fetch_linkedin_profiles_by_url(apify_token, new_xray_urls)
    ...
    for profile, source in candidate_pool:
        ...
        st.session_state.approved_candidates.append({
            ...
            "AI summary": eval_result.get("summary", ""),
        })
...
if st.session_state.stage == "done":
    ...
    if st.button("🔄 Почати новий пошук"):
        for key in ["stage", "trigger_run", "pending_note", "approved_candidates", "log", "search_history", "seen_urls", "iteration"]:
